(function(window,undefined){
	...
	nick[attr_fn] = nick[attr_prototype] = {
		...
		init:function(selector,context){
			...
			if(!isString(selector)){
				...
				each(selector && selector[attr_length] ? selector : [selector],function(i){
					
					!isElement($this = this) || self[attr_push]($this.body || (isWindow($this) ? selector : $this))
				...
				if(/<(\w+).*?>(.*?)(<\/\1>)/[attr_test](selector)){
					...
					self[attr_push][attr_apply](self,self.create(selector))
    ...
    (function(){
    	...
    	var attr_nodeName = 'nodeName';
    	
    	var attr_table = 'table';
    	...
    	var find = function(myself,selector,relation,all){
    		...
    		bind(myself,function(){
    			...
    			elements = relation ? $this[relation] : isElement(selector) ? [selector] : $this[attr_querySelectorAll](selector);
    			
    			if(isElement(selector)) selector = empty;
    	...
    	var createElement = function(html,getTextNode,runScript){
    		...
    		var div = document[attr_createElement](attr_table);
    		...
    		nodeList[attr_push][attr_apply](nodeList,toArray(div[getTextNode ? attr_childNodes : attr_children]));
    		
    		//遍历所有节点
    		$.each(nodeList, function(i) {
    			//如果节点类型为tbody则删除当前节点 并在此节点位置 插入tbody中的所有子节点
    			if(this[attr_nodeName] == 'TBODY'){
    				
    				nodeList.splice.apply(nodeList,[i,1][attr_concat](toArray(this[attr_childNodes])));
    			}
    		});
    		...
    		!runScript || each(nodeList,function(i){
    			...
    			if(self.tagName == attr_script[attr_toUpperCase]()){
	    			...
	    			each([self[attr_src] ? attr_src:'',attr_type,attr_charset],function(){
	    			...
	    			if(!self[attr_src]){
	    				...
	    				$this[attr_innerHtml] = self[attr_innerHtml]
	    			}
    	...
    	var insertElement = function(myself,elements,relation,replace){
    		...
    		each(elements,function(i){
    			...
    			newNode = isString(newNode = this) || type(newNode) == 'number' ? createElement(newNode,fake,real) : !newNode ? fake : newNode[attr_length]!==undefined && !callback ? newNode : [newNode];
    		...
    		each(myself,function(i){
    			...
    			if($this[attr_nodeName] == attr_table[attr_toUpperCase]()) $this = $this[attr_firstElementChild];
    		...
    		index:function(selector){
    			...
    			if(selector && !isString(selector)){
    				...
    				return each(this,function(i){
    					...
    					if((selector[0] || selector) == this) return i+1
    ...
    (function(){
        ...
        var css = function(myself,attr,value){
        	...
        	if(!$this || !$this[attr_style]) return myself;
        ...
        var eventsCallback = function($this,type,event,allEvents,x,y){
    		...
    		do{
    			...
    			each(allEvents,function(){
					...
					each(this,function(selector){
						...
						if(selector ?  target[attr_matchesSelector](selector) : target == $this || isWindow($this)){
    		...
    		//List traversal process after the event
   			each(events,function(){
   				...
   				each(self[1],function(){
    				...
    				//alert(extend({},event,{type:type})[attr_preventDefault])
   					this[attr_call](self[0],extend({},event,{type:type}),x,y) !== fake || event[attr_preventDefault]();
   		...
   		var gestureEvent = function($this,attr_subline){
    		...
    		var attr_startTime = attr_events_info+attr_start+'time';
    		...
    		$this[attr_addEventListener](attr_touch+attr_end,function(event){
    			...
    			touchTime = $this[attr_touchTime] || 0;
    			
    			touchLongTime = $this[attr_touchLongTime] || 3000;
    			...
    			if(!moveX && !moveY){
    				...
    				type = useTime>touchTime && useTime<touchLongTime ? (isApp?'tap':empty) : useTime>touchTime ?'longclick' : empty;
				...
				if(type) eventsCallback($this,type,event,[$this[attr_events][type],$this[attr_events][type+attr_subline]],moveX,moveY);
   		...
   		var addEventListener = function(myself,type,callback,selector,bool){
   			...
   			return bind(myself,function(){
   				...
   				each(type,function(){
					...
					//清除事件类型中的后缀,无论是否委托其实都只绑定了一次事件
   					...
   					//如果当前元素未绑定此此事件类型则绑定事件，移动设备的手势事件也是click避免重复绑定因此作出判断
   					//If the current element binding the this event type is not binding, gesture events of mobile devices is also click to avoid repetition and binding judgment
		...
		(function(){
			...
			var parseAnimationOptions = function(style,options){
	   		...
	   		var commonAnimation = function(style,display,options,callback,opacity){
				...
				//如果动画未执行完毕则设置元素显示，无论是显示或隐藏动画，开始显示时的显示模式应为显示状态
				//If the animation is not complete, set elements, according to whether to show or hide the animation, began to show the display mode should be display status
				
				if(!isComplete)css(self,attr_display,showStyle[attr_display]);
	   		...
	   		each('fade slide hide show toggle'[attr_split](pattern_space),function(i){
	   			...
	   			each(types[i] ? types[i][attr_concat](['Toggle']) : [empty],function(j){
	   				...
	   				nick[attr_fn][method] = function(options,callback,opacity){
	   					...
	   					if(i>1 && isObject(options) && options[attr_time] === undefined) options[attr_time] = 0 ;
    	...
    	(function(){
    		...
    		each(self = [' dbl long'[attr_split](pattern_space),('over enter out leave move'+updown)[attr_split](pattern_space),'start end move cancel'[attr_split](pattern_space),[empty][attr_concat](direction),updown[attr_split](pattern_space),'load error change scroll resize tap message'[attr_split](pattern_space)  ],function(i){
    			...
    			each(this,function(j){
    				...
    				nick[attr_fn][type = i ? typePrefix+type : type+typePrefix] = function(callback,bool){
    					
    					return addEventListener(this,type,callback,bool)
    		...
    		each('focus blur change'.split(' '),function(){
    			
    			var type = this;
    			
    			nick[attr_fn][type] = function(callback,bool){
	...
	(function(){
		...
		var param = function(data,depth){
			...
			map(data,function(i,path){
				...
				if(path)str+=path+'='+encodeURIComponent(this)+'&';
		...
		var ajax = function(options){
			...
			var startQueue = function(){
				...
				if($this[attr_queue_length]<4 && queue[attr_length]){
					...
					xmlrequest.send(options[attr_type] == attr_get ? vacancy : options[attr_data]);
					...
					setTimeout(function(){
						...
						if(xmlrequest[attr_readyState] != 4) xmlrequest.abort();
					...
					xmlrequest.onreadystatechange = function(){
						...
						if(xmlrequest[attr_readyState] == 4){
							...
							complte = status>=200 && status<300 || status==304 || status == 0;
			...
			var parseOptions = function(){
				...
				headers['Content-'+attr_type] = (isGet?'text/plain;':'application/x-www-form-urlencoded;')+(options[attr_charset] ? attr_charset+'='+options[attr_charset]:'');
				...
				if(isGet && options[attr_data])options[attr_url]+=attr_and+options[attr_data];
		...
		var attr_cookie = 'cookie';
		
		var attr_ready = attr_readyState[attr_substr](0,5);
		...
		nick.toArray = toArray;
		
		//批量遍历并绑定方法
		//Batch traversal and binding method
		
		each({camelCase:camelCase,type:type,isNick:isNick,each:each,inArray:inArray,ucword:ucword,parseJSON:parseJSON,map:map,param:param,cover:cover,callback:callback,ajax:ajax,array2Object:array2Object,toNumber:toNumber,extend:extend,isWindow:isWindow,browser:browser},function(i){
		...
		//绑定each方法用于遍历自身元素
		//
		nick[attr_fn].each = function(callback){
			
			return bind(this,callback)
			
		}
		
		//绑定加载事件队列，默认绑定dom加载事件
		//绑定加载事件队列，默认绑定dom加载事件
		
		nick[attr_ready] = function(callback,options){
			
			options = cover({
				
				//绑定事件的元素
				//The element of the binding event
				element:document,
				...
				//要查询的目标元素
				//The target element to query
				target:document,
				
				//要查询的属性
				//Properties to query
				attr:attr_readyState,
			...
			var sid = setInterval(function(){
				...
				if( (value === empty ? target[attr] : target[attr] == value) || num>max){
					
					if(num<=max) callback[attr_call](self);
		...
		nick[attr_fn][attr_ready] = function(callback,options){
			
			return bind(this,function(){
				
				var self = this;
				
				var attr_contentWindow = 'contentWindow';
				
				nick[attr_ready][attr_call](self,callback,{element:self[attr_contentWindow] ? self[attr_contentWindow].document : self})
		...
		nick[attr_cookie] = function(name,value,time){
			// 获取cookie  添加cookie  删除cookie   设置生存周期
			
			var cookies = document[attr_cookie]+empty;
			
			if(value === undefined){
				
				 var startIndex =cookies[attr_indexOf](name + '=');
				 
				 if(startIndex>-1){
				 	
				 	startIndex+=name[attr_length]+1;
				 	
				 	var endIndex = cookies[attr_indexOf](';',startIndex);
				 	
				 	endIndex = endIndex<0 ? cookies[attr_length] :endIndex;
				...
				document[attr_cookie]=name+ '=' +escape(value)+(!time ? "" : ";expires="+exdate.toGMTString())
